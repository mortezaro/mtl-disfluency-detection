...
class SequenceTagger(object):
    ...
    def vocab_creator(self, data_train, data_dev, data_test, embedding_path=None):
    ...
    def network_creator(self):
        ...
        self.timings = tf.placeholder(tf.int32, [None, None], name="timings")
        ...
        if self.config["char_embedding_size"] > 0 and self.config["char_recurrent_size"] > 0:
            with tf.variable_scope("chars"), tf.control_dependencies([tf.assert_equal(tf.shape(self.char_ids)[2], tf.reduce_max(self.word_lengths), message="Char dimensions don't match")]):
                ...
                if self.config["lmcost_char_gamma"] > 0.0:
                    self.loss += self.config["lmcost_char_gamma"] * self.naive_cost_creator(char_output_tensor, char_output_tensor, self.sentence_lengths, self.word_ids, "separate", "lmcost_char_separate")
                if self.config["lmcost_joint_char_gamma"] > 0.0:
                    self.loss += self.config["lmcost_joint_char_gamma"] * self.naive_cost_creator(char_output_tensor, char_output_tensor, self.sentence_lengths, self.word_ids, "joint", "lmcost_char_joint")
        ...
        if self.config["lmcost_lstm_gamma"] > 0.0:
            self.loss += self.config["lmcost_lstm_gamma"] * self.naive_cost_creator(lstm_outputs_fw, lstm_outputs_bw, self.sentence_lengths, self.word_ids, "separate", "lmcost_lstm_separate")
        if self.config["lmcost_joint_lstm_gamma"] > 0.0:
            self.loss += self.config["lmcost_joint_lstm_gamma"] * self.naive_cost_creator(lstm_outputs_fw, lstm_outputs_bw, self.sentence_lengths, self.word_ids, "joint", "lmcost_lstm_joint")
        ...
        self.train_op = self.optimizer_creator(self.config["opt_strategy"], self.loss, self.learningrate, self.config["clip"])


    def naive_cost_creator(self, input_tensor_fw, input_tensor_bw, sentence_lengths, target_ids, lmcost_type, name):
        with tf.variable_scope(name):
            ...
            if lmcost_type == "separate":
                ...
                lmcost_fw = self._naive_cost_creator(input_tensor_fw[:,:-1,:], lmcost_max_vocab_size, lmcost_fw_mask, target_ids[:,1:], name=name+"_fw")
                lmcost_bw = self._naive_cost_creator(input_tensor_bw[:,1:,:], lmcost_max_vocab_size, lmcost_bw_mask, target_ids[:,:-1], name=name+"_bw")
                ...
                cost += self._naive_cost_creator(joint_input_tensor, lmcost_max_vocab_size, lmcost_mask, target_ids[:,1:-1], name=name+"_joint")
            else:
                raise ValueError("Unknown lmcost_type: " + str(lmcost_type))
            return cost


    def _naive_cost_creator(self, input_tensor, lmcost_max_vocab_size, lmcost_mask, target_ids, name):
        with tf.variable_scope(name):
    ...
    def optimizer_creator(self, opt_strategy, loss, learningrate, clip):
        optimizer = None
    ...
    def pretrained_embeddings(self, embedding_path):
    ...
    def word_to_id(self, token, token2id, unk_token, lowercase=False, replace_digits=False, singletons=None, singletons_prob=0.0):
        if lowercase == True:
            token = token.lower()
    ...
    def create_input_dictionary(self, batch, is_training, learningrate):
        ...
        for i in range(len(batch)):
            for j in range(len(batch[i])):
                word_ids[i][j] = self.word_to_id(batch[i][j][0], self.word2id, self.UNK, lowercase=self.config["lowercase"], replace_digits=self.config["replace_digits"], singletons=singletons, singletons_prob=singletons_prob)
                label_ids[i][j] = self.word_to_id(batch[i][j][-1], self.label2id, None)
                word_lengths[i][j] = min(len(batch[i][j][0]), max_word_length)
                for k in range(min(len(batch[i][j][0]), max_word_length)):
                    char_ids[i][j][k] = self.word_to_id(batch[i][j][0][k], self.char2id, self.CUNK)
    ...
    def viterbi(self, score, transition_params):
    ...
    def process(self, batch, is_training, learningrate):
        feed_dict = self.create_input_dictionary(batch, is_training, learningrate)
        ...
        if self.config["crf_on_top"] == True:
            ...
            for i in range(len(batch)):
                ...
                viterbi_seq, viterbi_score, viterbi_trellis = self.viterbi(scores[i], self.session.run(self.crf_transition_params))
    ...
    def load(filename):
        with open(filename, 'rb') as f:
            ...
            labeler = SequenceTagger(dump["config"])
